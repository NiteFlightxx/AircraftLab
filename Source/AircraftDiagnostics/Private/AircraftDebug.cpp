	...
	const FAircraftFlightControllerRuntimeConfig& Config)
{
	...
	UE_LOG(LogAircraft, Log,
		...
		*GetNameSafe(Component.GetOwner()), SimulationLOD, *RootBone.ToString(),
		...
		Constraint.GetOrientationDriveSLERP() ? 1 : 0,
		Constraint.GetAngularVelocityDriveSLERP() ? 1 : 0,
		...
		Constraint.ProfileInstance.AngularDrive.GetAccelerationMode() ? 1 : 0);
}
